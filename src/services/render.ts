...
function layout(title: string, active: string, body: string) {
  const nav = [
    { href: "/", label: "Overview" },
    { href: "/fleet-lane", label: "Fleet Lane" },
    { href: "/compliance-risks", label: "Compliance Risks" },
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Endpoint Compliance Control Plane</div>
        <div class="right">
          <div>tenant posture · synthetic intune device packets</div>
          <div>azure · microsoft 365 · intune · endpoint ops</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            ...
            <span class="meta-chip">Synthetic Microsoft Graph managedDevices packets</span>
          </div>
          <h1>Intune device compliance, stale sync drift, and remediation posture that stay operator-readable.</h1>
          <p>This control plane turns raw Microsoft Graph Intune exports into a buyer-readable endpoint posture surface for platform, security, and IT operations teams: noncompliant devices, stale sync risk, encryption gaps, rooted BYOD devices, and the remediation packet needed before access and rollout windows break.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Enterprise endpoint compliance for Intune, Microsoft 365, and platform-security teams.</strong><br />Audit-safe fleet visibility, remediation sequencing, and BYOD posture without tenant-secret exposure.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline analyzer + CLI + dashboard surface.</strong><br />This repo includes a reusable analyzer that reads Microsoft Graph <code>managedDevices</code> exports and turns them into fleet posture, risk, and remediation packets.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>Azure / Microsoft 365 / Entra / Intune</strong> should see real endpoint operations work: device compliance, stale sync drift, encryption policy, and BYOD controls.</p>
          </div>
        </aside>
      </div>
      ${body}
      <div class="footer">
        <div>intune-device-compliance-ops · synthetic sample data only</div>
        <div>routes: / · /fleet-lane · /compliance-risks · /remediation-posture · /verification · /docs</div>
      </div>
    </div>
  </body>
</html>`;
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Operator Snapshot</h2><div class="note">managedDevices export lens</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.devices}</div><div class="lbl">managed devices</div><div class="h">Synthetic endpoint records with company-owned, BYOD, and kiosk posture.</div></div>
          ...
          <div class="kpi amber"><div class="v">${metrics.staleDevices}</div><div class="lbl">stale sync findings</div><div class="h">Devices whose last Intune sync has drifted beyond the stale threshold.</div></div>
          <div class="kpi plum"><div class="v">${metrics.byodDevices}</div><div class="lbl">byod scope</div><div class="h">Personal devices under corporate compliance policy and review pressure.</div></div>
      ...
      <section class="section">
        <div class="sh"><h2>Why operators care</h2><div class="note">endpoint posture · stale sync · remediation packet</div></div>
        <div class="stack">
          ...
          <div class="src"><div class="src-name">device evidence</div><div class="src-tit">Turn exports into operator proof</div><p>Every fleet lane stays tied to owner, business role, compliance state, and the next concrete remediation move.</p></div>
          <div class="src"><div class="src-name">recruiter signal</div><div class="src-tit">Show Microsoft admin depth</div><p>This is real Intune / Microsoft 365 endpoint-operations proof, not a generic cloud keyword page.</p></div>
        </div>
      </section>`
  );
}

export function renderFleetLane() {
  return layout(
    "Intune Device Compliance Ops — Fleet Lane",
    "/fleet-lane",
    `<section class="section">
        <div class="sh"><h2>Fleet Lane</h2><div class="note">owner · device class · compliance posture</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Device</th>
...
export function renderComplianceRisks() {
  return layout(
    "Intune Device Compliance Ops — Compliance Risks",
    "/compliance-risks",
    `<section class="section">
        <div class="sh"><h2>Compliance Risks</h2><div class="note">severity · owner · device</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Risk</th>
...
export function renderRemediationPosture() {
  return layout(
    "Intune Device Compliance Ops — Remediation Posture",
    "/remediation-posture",
    `<section class="section">
...
export function renderVerification() {
  return layout(
    "Intune Device Compliance Ops — Verification",
    "/verification",
    `<section class="section">
        ...
        <div class="stack">
          ${verification()
            .map(
              (item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface is built to stay honest about offline exports, synthetic sample data, and real Microsoft endpoint operations posture.</p></div>`
            )
            .join("")}
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Intune Device Compliance Ops — Docs",
    "/docs",
    `<section class="section">
        <div class="sh"><h2>Docs</h2><div class="note">routes · cli · api</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">routes</div><div class="src-tit">Public control surface</div><p><code>/</code>, <code>/fleet-lane</code>, <code>/compliance-risks</code>, <code>/remediation-posture</code>, <code>/verification</code>, <code>/docs</code></p></div>
          <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/fleet-lane</code>, <code>/api/compliance-risks</code>, <code>/api/remediation-posture</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline Graph export analysis</div><p><code>npx intune-device-compliance fixtures/devices-clean.json --format summary</code> renders the same underlying analyzer posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
